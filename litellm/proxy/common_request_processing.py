...
async def open_sse_before_first_byte(
    ...
    on_late_failure: Callable[[Exception], Awaitable[None]] | None = None,
    ...
    async def keepalive_then_relay() -> AsyncGenerator[bytes, None]:
        try:
            ...
            try:
                ...
                # would never fire and the failure would go unaudited.
                if on_late_failure is not None:
                    with contextlib.suppress(Exception):
                        await on_late_failure(exc)
...
class ProxyBaseLLMRequestProcessing:
    ...
    async def base_process_llm_request(
        ...
        async def _audit_late_failure(exc: Exception) -> None:
